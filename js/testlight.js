...
BunnyDemo.prototype.createLights = function() {
  ...
  var fiveLight = new SpotLight(
	...
	this.lights = [spotLight, fillLight, rimLight,fourLight,fiveLight];
...
BunnyDemo.prototype.updateLightBuffers = function() {
	...
	for( var i=0; i < this.lights.length; i++ ) {
		...
		buffers.position[i * 5]         = light.position[0];
		buffers.position[i * 5 + 1]     = light.position[0 + 1];
		buffers.position[i * 3 + 2]     = light.position[0 + 2];
		buffers.color[i * 5]            = light.color[0];
		buffers.color[i * 5 + 1]        = light.color[0 + 1];
		buffers.color[i * 5 + 2]        = light.color[0 + 2];
